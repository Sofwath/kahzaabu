...
#
# SCOPING BOUNDARY (important for future readers):
#   This table is PER-USER-INSTALL, not multi-tenant. Hermes' design
#   is one-user-per-process and session_id is a UUID generated by
#   hermes — so the practical collision risk between users sharing
#   a hermes deployment is zero (the session_id namespace is global
#   per the UUID guarantee, not scoped by user). If a future
#   deployment shares ONE kahzaabu DB across MULTIPLE users (rare;
#   would need an extra-careful security review for a fact-check
#   archive that mentions a head of state), the table would need
#   a (user_id, session_id) compound PK and a per-user partition.
#   That's an explicit non-goal of the reference implementation —
#   see DISCLAIMER.md and ADR 0013.
